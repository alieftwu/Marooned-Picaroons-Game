...
func _input(event): #select path to move on using mouse
...
func _physics_process(delta): #move along path
	if current_id_path.is_empty():
		return
	
	var target_position = tile_map.map_to_local(current_id_path.front())
	
	global_position = global_position.move_toward(target_position, 1) #the 1 in this line needs to be changed to speed
